 ...
 * Incluye la lógica para determinar el estado de "Socio Moroso".
 */
class ClientesAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ClienteViewHolder {
        // Inicializar el DAO aquí asegura que el contexto esté disponible
        if (!::clienteDao.isInitialized) {
            clienteDao = ClienteDao(parent.context)
        }
    ...
    override fun onBindViewHolder(holder: ClienteViewHolder, position: Int) {
        ...
        if (cliente.asociarse) {
            // Es Socio, ahora verificamos si es Moroso.
            // NOTA: Esta llamada a la DB en el hilo principal es solo para fines de
            // este ejercicio. En un proyecto real, se usaría Coroutines/AsyncTask.
            ...
            if (socioDetails != null && isCuotaVencida(socioDetails.fechaVencimientoCuota)) {
                ...
                holder.statusIndicator.setImageResource(R.drawable.ic_status_moroso) // Necesitas crear este drawable
                holder.tvSocioStatus.setTextColor(ContextCompat.getColor(context, R.color.color_moroso)) // Necesitas definir este color
            } else {
                // Estado: ACTIVO
    ...
    private fun isCuotaVencida(fechaVencimiento: String): Boolean {
        return try {
            ...
            // Si el formato de fecha es incorrecto o nulo, asumimos que no es moroso para evitar crash
            false
        }
    }
